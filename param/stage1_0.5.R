...
"
writeLines(modelString, con = "stage1_0.5.bug")
...
samples.stage1 <- foreach(j = 1:3) %dopar% {
  cat(paste0("---- Chain ", j, " started ----\n"))
  
  inits.jags <- list(list(.RNG.seed = sample(1:10000, 1), .RNG.name = "base::Mersenne-Twister"))
  
  model.jags <- jags.model("stage1.bug", data = data.jags, inits = inits.jags, n.chains = 1, n.adapt = 5000)
  cat(paste0("Chain ", j, ": model compiled and adapted\n"))
  
  mcmc.samples.stage1 <- coda.samples(model.jags, variable.names = parameters.jags, n.iter = 10000, thin = 10)
  cat(paste0("Chain ", j, ": sampling complete\n"))
  
  mcmc.samples.stage1[[1]]
  
}

save(samples.stage1, file = "samples_stage1_0.5.Rdata")
