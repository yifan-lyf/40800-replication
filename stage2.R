...
writeLines(modelString, con = "stage2.bug")
...
samples.stage2 <- foreach(j = 1:N.draws) %dopar% {
  ...
  model2.jags <- jags.model("stage2.bug", data = data2.jags, inits = inits.jags, n.chains = 1, n.adapt = 50000)
...
save(samples.stage2, file = "samples_stage2.Rdata")
